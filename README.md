...
`Recurring.StartDate` is required by the Payment API. It comes from the `startDate` design property when one is set; otherwise it defaults to today's date (`yyyy-mm-dd`, payer's local time).
